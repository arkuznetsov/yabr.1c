...
Перем НакопленныеДанные;       // Массив                 - буфер данных для сохранения в файл JSON
...
Процедура УстановитьПараметрОбработкиДанных(Знач ИмяПараметра, Знач Значение) Экспорт
	...
	Если ВРег(ИмяПараметра) = ВРег("ПутьКФайлу") Тогда
		ПутьКФайлу = Значение;
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ОбрабатыватьМассивПоэлементно") Тогда
...
Процедура ДобавитьЗапись(Элемент)
	...
	Если ТипЗнч(Элемент) = Тип("Массив") И ОбрабатыватьМассивПоэлементно Тогда
		Для Каждого ТекЭлемент Из Элемент Цикл
			НакопленныеДанные.Добавить(ТекЭлемент);
		КонецЦикла;
	Иначе
