...
// Функция - возвращает значение поля описания ИБ
//
// Параметры:
//    Имя           - Строка     - имя или идентификатор ИБ
//    Поле          - Строка     - имя поля описания ИБ (по умолчанию "Ид")
//
// Возвращаемое значение:
//    Произвольный     - значение поля описания ИБ
//
Функция ЗначениеПоляОписанияИБ(Знач Имя = Неопределено, Знач Поле = "Ид") Экспорт
	...
	Для Каждого ТекОписание Из НакопленныеДанные Цикл
		Если ТекОписание.Имя = Имя ИЛИ ТекОписание.Ид = Имя Тогда
			НайденноеОписание = ТекОписание;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Возврат НайденноеОписание[Поле];
	
КонецФункции // ЗначениеПоляОписанияИБ()
...
Функция ДобавитьЗапись(Элемент)
	...
	Запись.Вставить("КаталогЖР"        ,
	                СтрШаблон("%1\%2\1Cv8Log", Запись.Кластер_Каталог, Запись.Ид));
	Запись.Вставить("СловарьЖР"        ,
	                СтрШаблон("%1\%2\1Cv8Log\1cv8.lgf", Запись.Кластер_Каталог, Запись.Ид));
