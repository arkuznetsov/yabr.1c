Перем МенеджерОбработкиДанных; // ВнешняяОбработкаОбъект - обработка - менеджер
...
Перем ПрочитанныеДанные;
Перем ПоказателиЧтенияДанных;
...
Функция РезультатыОбработки() Экспорт
	...
	Возврат Новый Структура("Данные, Показатели", ПрочитанныеДанные, ПоказателиЧтенияДанных);
...
Процедура ПрочитатьСкобкоФайл(Знач Данные = Неопределено)
	...
	ПрочитанныеДанные = ИнициализироватьЭлемент(Неопределено);
	...
	Пока НЕ ДанныеСтроки = Неопределено Цикл
		...
		Если НомерСтроки % 1000 = 0 Тогда
			...
			ПоказателиЧтенияДанных = Новый Структура("Скорость, ТекСкорость, Прочитано",
			                                         Скорость, ТекСкорость, НомерСтроки)
		КонецЕсли;
