...
Функция ОписаниеПараметров() Экспорт
	...
	ДобавитьОписаниеПараметра(Параметры,
	                          "ЗапоминатьНомераСтрок",
	                          "Булево",
	                          ,
	                          Ложь,
	                          "Истина - при чтении файла в результат будут добавлены номера прочитанных строк;
                              |Ложь - номера строк запоминаться не будут.");
	ДобавитьОписаниеПараметра(Параметры,
	                          "УровниЗаписей",
	                          "Строка, Массив",
	                          ,
...
Процедура УстановитьПараметрОбработкиДанных(Знач ИмяПараметра, Знач Значение) Экспорт
	...
	Если ВРег(ИмяПараметра) = ВРег("ЗапоминатьНомераСтрок") Тогда
		ЗапоминатьНомераСтрок = Значение;
...
Процедура УстановитьДанные(Знач ВходящиеДанные) Экспорт
	...
	Если ТипЗнч(ВходящиеДанные) = Тип("Строка") И ВРег(Лев(ВходящиеДанные, 18)) = ВРег("e1cib/tempstorage/") Тогда
		Данные = ПолучитьИзВременногоХранилища(ВходящиеДанные).ОткрытьПотокДляЧтения();
	Иначе
...
Функция ИмяФайла(Знач ДобавитьРасширение = Истина) Экспорт
	...
	Если ТипЗнч(Данные) = Тип("Строка") Тогда
		Если ВРег(Лев(Данные, 18)) = ВРег("e1cib/tempstorage/") Тогда
			Возврат "";
		Иначе
	...
	Пока НЕ ДанныеСтроки = Неопределено Цикл
		...
		Пока НЕ (СтрокаДляОбработкиПрочитана ИЛИ ДанныеСтроки = Неопределено) Цикл
			Если НЕ НомераСтрок.Свойство("НачСтрока") Тогда
				НомераСтрок.Вставить("НачСтрока", НомерСтроки);
			КонецЕсли;
			Если НомераСтрок.Свойство("КонСтрока") Тогда
				НомераСтрок.КонСтрока = Макс(НомераСтрок.КонСтрока, НомерСтроки);
			Иначе
				НомераСтрок.Вставить("КонСтрока", НомерСтроки);
			КонецЕсли;
			Если ЗапоминатьНомераСтрок Тогда
				НомераСтрок.Вставить(СтрШаблон("_%1", Формат(НомерСтроки, "ЧН=; ЧГ=;")), НомерСтроки);
			КонецЕсли;
...
Процедура ДополнитьНомераСтрокРодителей(Знач Элемент, Знач НомераСтрок = Неопределено)
	...
	Если ЗапоминатьНомераСтрок Тогда
		Для Каждого ТекПара Из НомераСтрок Цикл
			Если ТекПара.Ключ = "НачСтрока" ИЛИ ТекПара.Ключ = "КонСтрока" Тогда
				Продолжить;
			КонецЕсли;
	...
	Пока НЕ ТекЭлемент = Неопределено Цикл
		ТекЭлемент.НачСтрока = ?(ТекЭлемент.НачСтрока = 0, НомераСтрок.НачСтрока, Мин(ТекЭлемент.НачСтрока, НомераСтрок.НачСтрока));
