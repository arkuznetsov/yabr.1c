...
//
&НаСервере
Процедура ПрочитатьНаСервере(ПутьКФайлу)
	
	АдресДанных = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ПутьКФайлу), ЭтотОбъект.УникальныйИдентификатор);
	...
	ПараметрыОбработки.Вставить("ПутьКФайлу"                     , АдресДанных);
