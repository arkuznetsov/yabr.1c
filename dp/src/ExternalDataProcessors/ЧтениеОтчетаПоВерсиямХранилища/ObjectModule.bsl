...
Функция ДобавитьЗапись(Элемент)
	...
	Если ТипЗнч(ЗаписьДляДобавления) = Тип("Структура") Тогда
		...
		Возврат Истина;
	ИначеЕсли ОписаниеЯчейки.Строка = КоличествоСтрок И ОписаниеЯчейки.Колонка = 2 Тогда
		БуферЗаписи.Удалить("ТекущееПоле");
...
Функция ПолучитьОписаниеЯчейки(Элемент)
	...
	КонтейнерЯчеек1 = Элемент.Родитель.Родитель.Родитель.Значения;
	КонтейнерЯчеек2 = Элемент.Родитель.Родитель;
	 
	ИндексКолонки = КонтейнерЯчеек1[КонтейнерЯчеек2.Индекс - 1];
	...
	ВремИндекс = КонтейнерЯчеек2.Индекс - 4;
	
	Пока ВремИндекс > 0 Цикл
		Если ТипЗнч(КонтейнерЯчеек1[ВремИндекс]) = Тип("Строка")
		   И ТипЗнч(КонтейнерЯчеек1[ВремИндекс + 1]) = Тип("Строка") 
		   И ТипЗнч(КонтейнерЯчеек1[ВремИндекс + 2]) = Тип("Строка") 
		   И ТипЗнч(КонтейнерЯчеек1[ВремИндекс + 3]) = Тип("Строка") Тогда
			ИндексСтроки = КонтейнерЯчеек1[ВремИндекс];
			Прервать;
		КонецЕсли;
